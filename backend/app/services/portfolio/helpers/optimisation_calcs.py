...
def optimise_portfolio(mu_dict, cov_dict, w_baseline_dict, risk_aversion=0.5, baseline_reg=0.1, min_weight=0.0, max_weight=0.1):
    ...
    # Convert result to pandas Series keyed by symbols
    return pd.Series(w.value, index=symbols).to_dict()